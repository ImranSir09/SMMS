...
const Toast: React.FC<ToastProps> = ({ toast, onRemove }) => {
    return (
        <div className={`w-full max-w-sm p-3 rounded-lg shadow-lg flex items-start text-white border-l-4 ${COLORS[toast.type]} animate-fade-in-up`}>
...
export const ToastContainer: React.FC<{ toasts: ToastMessage[], removeToast: (id: number) => void }> = ({ toasts, removeToast }) => {
  return (
    <div className="fixed top-16 right-0 left-0 sm:left-auto sm:right-4 z-[100] p-4 sm:p-0 space-y-2 flex flex-col items-center sm:items-end">
        {toasts.map(toast => (
