...
import FormativeAssessmentRecordSheet from '../components/FormativeAssessmentRecordSheet';
import { SUBJECTS, ACADEMIC_YEAR } from '../constants';

const FormativeAssessmentRecord: React.FC = () => {
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            ...
            const records = await db.detailedFormativeAssessments.where({ studentId, subject: selectedSubject, academicYear: ACADEMIC_YEAR }).toArray();

            await generatePdfFromComponent(
                <FormativeAssessmentRecordSheet
                    student={student}
                    schoolDetails={schoolDetails}
                    subject={selectedSubject}
                    records={records}
                    session="2025" // As per image
                />,
                `Formative-Record-${student.name}-${selectedSubject}`
            );
    ...
    return (
        <div className="flex flex-col gap-4 animate-fade-in">
            <Card className="p-3">
                <h2 className="text-md font-semibold mb-2">Generate Formative Assessment Record</h2>
...
export default FormativeAssessmentRecord;