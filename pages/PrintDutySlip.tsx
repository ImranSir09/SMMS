...
const PrintDutySlip: React.FC = () => {
    const { schoolDetails } = useAppData();
    const [printableData, setPrintableData] = useState<{ staff: Staff; duty: { description: string; date: string } } | null>(null);
     ...
     const handleDownloadPdf = async () => {
        if (printableData && schoolDetails) {
            await generatePdfFromComponent(
                <DutySlip staff={printableData.staff} schoolDetails={schoolDetails} dutyDetails={printableData.duty} />,
                `Duty-Slip-${printableData.staff.staffId}-${printableData.duty.date}`
            );
        }
    };
    
    if (!printableData || !schoolDetails) {
        return <div>No data found for printing. Please generate the slip again from the certificates page.</div>;
    }
    ...
    const ControlPanel = () => (
      <div className="max-w-4xl mx-auto mb-4 p-4 bg-white rounded-lg shadow-md print:hidden">
        ...
        <p className="text-gray-600 mb-4">Preview for {printableData.staff.name}'s Duty Slip.</p>
        ...
        <div className="bg-gray-200 min-h-screen p-4 sm:p-8 print:p-0 print:bg-white">
            ...
            <style>{`
                ...
                @media print {
                    ...
                    #duty-slip, #duty-slip * { visibility: visible; }
                    #duty-slip { position: absolute; left: 0; top: 0; width: 100%; height: 100%; }
                }
                .A4-page-container {
                    display: flex;
